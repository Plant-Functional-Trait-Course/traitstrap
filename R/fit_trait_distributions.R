...
#' @description Fits distributions to imputed traits and returns a tibble containining the fitted parameters.
...
#' @examples 
#' @export
fit_trait_distributions(imputed_traits, distribution_type = "normal"){
  ...
  if (any(!unique(distribution_type)%in%c("beta","normal","lognormal"))) {
    x = distribution_type[!distribution_type %in% c("beta","normal","lognormal")]
  ...
  if("beta" %in% distribution_type){
    ...
    beta_counts <- imputed_traits %>% 
      filter(.data[[trait_col]] == names(distribution_type)[distribution_type=="beta"]) %>% 
      group_by_at(c(as.character(scale_hierarchy),trait_col,taxon_col)) %>% 
      summarise(n=n(),.groups="keep") 
    
    if(any(beta_counts$n < 2)) {
    stop("Fitting a beta distrbution requires at least 2 points per distribution.
         We suggest re-imputing traits with a minimum of (at least) 2 leaves.") 
      }
    
    
    #check that values are between 0 and 1
    beta_vals <- imputed_traits %>% ungroup() %>%
      filter(.data[[trait_col]] == names(distribution_type)[distribution_type=="beta"]) %>% select( all_of(value_col) )
  ...
  if("lognormal" %in% distribution_type){
    ...
    ln_vals <- imputed_traits %>% ungroup() %>%
      filter(.data[[trait_col]] == names(distribution_type)[distribution_type=="lognormal"]) %>% select( all_of(value_col) )
  ...
  distribution_parms <- imputed_traits %>% 
    ...
    mutate(distribution_type = unlist(distribution_type[.data[[trait_col]]]), fit_sample_size = n()) %>% 
    summarize(fit_sample_size = n(), get_dist_parms(data = .data[[value_col]],distribution_type = distribution_type))
  
  #set arguments as attributes so next functions have access to them
  attr(distribution_parms,"attrib") <- attributes(imputed_traits)
  
  class(distribution_parms) <- c("parametric_distrbutions", class(distribution_parms))
