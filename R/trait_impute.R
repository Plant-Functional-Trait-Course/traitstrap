...
#' @param min_n_samples minimum number of samples with traits to accept before
#' searching for samples higher up the hierarchy.
#'
#' @description
#'
...
trait_impute <- function(
  ...
  out <- scale_hierarchy %>%
    map_dfr(~{
      ...
      result <- result %>%
       ...
       mutate(
         n_samples = n(),
         weight = .data[[abundance_col]] / .data$n_samples,
         level = scale_level
       )
  ...
  #get max number of samples available
  out <- out %>%
    ungroup() %>%
    group_by(across(all_of(c(as.character(scale_hierarchy),
                             taxon_col, trait_col, other_col)))) %>%
    mutate(max_n_samples = max(.data$n_samples))
  ...
  if (!keep_all) {#keep only finest scale trait data available
    out <- out %>%
      ...
      group_by(across(all_of(c(as.character(scale_hierarchy),
                               taxon_col, trait_col, other_col)))) %>%
      filter(
        #group has more than minimum number of samples
        .data$n_samples >= min_n_samples |
          # OR maximum number of samples when max is lower than required
       (.data$n_samples == .data$max_n_samples &
          .data$max_n_samples <= min_n_samples)
      ) %>%
