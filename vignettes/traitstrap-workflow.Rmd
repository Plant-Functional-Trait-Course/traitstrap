...
This function needs the output from the trait filling as `selected_traits`, number bootstrapped of replications `nrep`, bootstrap size `sample_size` and `id`.
...
tpd <- boot_tpd %>% 
  mutate(result = map(result, as.data.frame)) %>% 
  unnest(result) |> 
  pivot_longer(cols = species.FRichness:species.FDivergence,
               values_to = "value",
               names_to = "metric") |> 
