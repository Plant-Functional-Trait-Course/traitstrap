...
* **trait selection** function which allows to account for intraspecific trait variation and hierarchical sampling design.
...
The `trait_select` function uses a hierarchical sampling design, which allows to account for incomplete trait collections, traits from different spatial or temporal levels (i.e. local traits vs. databases), different taxonomic resolution and/or experimental design. 
...
With `scale_hierarchy` you can define the levels at which the traits have been collected and the order of trait selection starting with the highest level (e.g. global database, regional, site, plot).
In the example below we have the levels **Site** and **PlotID**, starting with the highest level.

The `trait_select` function will choose if available a trait value from the lowest level, i.e. species X from plot A in site 1.
...
If the minimum number is not reached (i.e. there are only 3 trait values at a specific level), trait values from the next higher level will be selected, to avoid sampling the same individual several times, which could result in unrealistic variances.
The default minimum number of samples is 5.

In the `other_col` argument other grouping variables in the community dataset can be defined and will be kept after the trait selection.


```{r trait-select, echo=TRUE, eval=TRUE}

trait_selection <- trait_select(
...
Traitstrap also allows to include taxonomy and experimental design in the trait selection step.

With the argument `taxon_col` the taxonomic hierarchy for sampling can be defined.
This means if traits for a specific species are not available, trait values from the same genus will be selected.
For this a list of the taxonomic hierarchy has to be defined (e.g. "Taxon", "Genus").
Note that traits from species of the same genus can have very different traits and it might not be meaningful to select these traits.
Therefore, you should always check the trait distributions for the same genus before using taxonomic trait selection.

The argument `treatment_col` allows to incorporate an experimental design where traits are selected from the same experimental treatment or the first factor level, which is assumed to be the control.
...
The selection step can be defined at certain level using the `treatment_level` argument.
Depending on the experimental design it might make sense to select traits at a certain level, e.g. block or site.

Here is an example how to include taxonomy and experimental design in the trait selection function (code not run).

```{r trait-select2, echo=TRUE, eval=FALSE}

trait_selection2 <- trait_select(
    comm = community,
    traits = trait,
...
The output of the trait selection function is then used to do a **nonparametric bootstrapping** using the `trait_np_bootstrap` function.
...
* an object of class selected traits (as produced by the function `trait_select`), and 
...
```{r multivariate, warning=FALSE, fig.width = 6}

library(FD)


multivariate_traits <- trait_select(comm = community %>%
...
boot_multi <- trait_multivariate_bootstrap(multivariate_traits,
                                            id = "ID",
...
# Because dbFD returns output as a list, we need to do a bit of work to wrangle it into a more useful format
# We'll extract Rao's Quadratic Entropy, since that performs well with bootstrapping.

boot_multi %>%
  rowwise()%>%
          mutate(RaoQ = .data$result$RaoQ) %>%
          dplyr::select(-result)-> raoq_est
  
raoq_est %>%
  ggplot(mapping = aes(x=RaoQ,
                       fill=PlotID))+
...
# for each replicated site, convert the distribution into a trait probability distribution and then derive functional diversity metrics

  boot_tpd <- trait_multivariate_bootstrap(multivariate_traits,
                                           id = "ID",
                                           nrep = 20, #Note that the number of reps is set low for demo purposes
                                           sample_size = 200,
                                           fun = function(x){TPDs(species = rep(1,200),
                                                                  traits = x) %>%
                                               REND(TPDs = .)})

# reformat the results to make plotting easier
  boot_tpd %>%
    rowwise()%>%
    mutate(FRichness = .data$result$species$FRichness,
           FEvenness = .data$result$species$FEvenness,
           FDivergence = .data$result$species$FDivergence) %>%
    dplyr::select(-result)%>%
    tidyr::pivot_longer(cols = c("FRichness","FEvenness","FDivergence"),
                        names_to = "metric") -> boot_tpd


boot_tpd %>%
  ggplot()+
  geom_violin(mapping = aes(y=value,x=PlotID, fill=PlotID),
              alpha=0.5,
...
Basically, this function summarizes from which level the traits are selected, and how much coverage of the community is reached.
...
# list missing traits
trait_missing(selected_trait = trait_selection,
              comm = community)
