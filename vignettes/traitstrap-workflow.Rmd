...
```{r hex, echo=FALSE, out.width='20%', fig.align = 'right'}
knitr::include_graphics("Traitstrap_hex.png")
```
...
Specifically, the **mean** and the **higher moments** (variance, skewness, and kurtosis) of trait distributions can reveal information about the various processes shaping species diversity.
Thus, while community ecology has typically focused on mean, the whole distribution contains useful information about community assembly, functioning, and dynamics.

```{r true-dist, echo=FALSE, out.width='80%'}
...
```{r comm-boot, echo=FALSE, out.width='80%'}
...
This approach allows to account for intraspecific variation and uses the hierarchical nature of trait variation across space, experimental treatments, and taxonomic resolution (i.e. species, genus) to select the most appropriate traits available.

The package has three **main functions**:

* **trait imputation** function which allows to account for intraspecific trait variation and hierarchical sawmpling desing.
...
The **trait data** should contain information about traits and trait values for as many species and individual in the community data as possible.
...
The trait_impute function will choose if available a trait value from the lowest level, i.e. species X from plot A in site 1.
If no trait value is available from that level, it will try the next group in the same level and choose a trait value from species X from plot B at site 1.
If there is no trait available, it will move up the hierarchy to the next level and choose a trait value from species X from site 2.
...
In the **other_col** argument other grouping variable in the community dataset can be 
defined and will be kept after the trait imputation.
...
Note that traits from species of the same genus can have very different traits and it might not be meanigful to impute these traits.
...
Here is an example how to include taxonomy and experimental desing in the trait imputation function (code not run).
...
If the raw data is extracted, nrep is forced to 1 to avoid memory issues, but if the user has a large amount of traits and individuals the raw data might need to be extracted from a subset of the whole dataset. 
...
One advantage of using a boostrapping approach, is that we get much more than a mean trait value.
We can in fact estimate the variance and other paramters of these trait distributions.
In traitstrap happy moments can be summarized and the confidence intervalls calcualted using the **trait_summarise_boot_moments** function.
The input variable for this function is the output from the nonparametric boostrapping function (or the parametric bootstrapping function, see below).

The confidence intervall can be calculated parametrically, using the mean and standard deviation, or nonparametrically using quantiles.
...
Currently the function supports normal, log-normal, and beta distributions.
...
In traitstrap both the parametric and nonparametric boostrapping funcitons allow returing raw trait distributions.

Note that if the data contains multiple traits, sites for several individuals, memory issues can occur and the raw trait distirbutions might have to be extracted from a subset of the data. 
...
```{r plot-raw-dist-np, echo=TRUE, eval=TRUE}
...
Based on simulations, we recommend to collect traits for at least 80% of the communitiy cover (Maitner et al. in prep).
