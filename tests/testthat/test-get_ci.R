...
test_that("get_ci ranks are appropriate", {
 ...
 expect_gt(object = get_ci(data = test_vector, which = "high"),
           expected = get_ci(data = test_vector, which = "low"))

 expect_gt(object = get_ci(data = test_vector, which = "high", sd_mult = 2),
           expected = get_ci(data = test_vector, which = "high"))

 expect_gt(object = get_ci(data = test_vector, which = "high", parametric = F),
           expected = get_ci(data = test_vector, which = "low", parametric = F))

 expect_gt(object = get_ci(data = test_vector, which = "low",
...
test_that("get_ci methods return same class and type", {
  test_vector <- runif(n = 100, min = 0, max = 100)
expect_equal(object = typeof(get_ci(data = test_vector, which = "high")),
...
test_that("get_ci data order doesn't matter", {
  ...
  expect_equal(object = get_ci(data = test_vector[100:1], which = "high"),
               expected = get_ci(data = test_vector[1:100], which = "high"))

  expect_equal(object = get_ci(data = test_vector[100:1],
                               which = "high", parametric = F),
               expected = get_ci(data = test_vector[1:100],
                                 which = "high", parametric = F))
